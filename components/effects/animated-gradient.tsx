 ...
 * passar AA em qualquer ponto (branco sobre #752996 8,35 · #4a1763 12+ · #c2407a 4,6).
 */
...
export const ATELIE_GRADIENTE: Params = {
  color1: '#752996',
  color2: '#c2407a',
  color3: '#4a1763',
  rotation: 114,
...
export function AnimatedGradient({
  ...
  useEffect(() => {
    ...
    // biome-ignore lint/correctness/useHookAtTopLevel: gl.useProgram é WebGL, não hook do React
    ...
    const draw = (time: number) => {
      if (runningRef.current) elapsedTotal += (time - last) / 1000
      last = time
    ...
    // biome-ignore lint/correctness/useExhaustiveDependencies: monta o contexto GL uma vez
  }, [params])
...
function hexToRgba(hex: string): [number, number, number, number] {
  ...
  if (hex.startsWith('rgba(')) {
    ...
    a = isHsla ? parseFloat(parts[3] ?? '0') : 1
    ;[r, g, b] = hslToRgb(h, s, l)
  } else if (hex.startsWith('#')) {
    const c = hex.slice(1)
    if (c.length === 3) {
      r = parseInt((c[0] ?? '0') + (c[0] ?? '0'), 16) / 255
      g = parseInt((c[1] ?? '0') + (c[1] ?? '0'), 16) / 255
      b = parseInt((c[2] ?? '0') + (c[2] ?? '0'), 16) / 255
    } else if (c.length >= 6) {
